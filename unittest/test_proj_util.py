...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    filename=f"unittest-{test_code}.log", filemode='a')
...
class TestCaseRandomCode(unittest.TestCase):
    def test_randomness(self):
        ...
        if code in code_list:
            ...
            logging.warning(f"{code} was repeated!")
...
class TestCaseWriteCSV(unittest.TestCase):
    def setUp(self):
        ...
        self.filename = f"{test_code}-CSV_test_write.csv"
    ...
    def test_csv_write(self):
        ...
        """
        logging.info("test_csv_write() initialized.")
        from os import path
        file_exists = path.isfile(self.filepath)
        self.assertTrue(file_exists)
        logging.info("test_csv_write() completed.")

    def test_csv_read(self):
        logging.info("test_csv_read() initialized.")
        file = open(self.filepath, "r")
        file_string = file.read()
        self.assertEqual("11,12,13,14\n21,22,23,24\n31,32,33,34\n", file_string)
        file.close()
        logging.info("test_csv_read() completed.")
